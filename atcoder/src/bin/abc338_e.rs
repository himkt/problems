...
fn main() {
    ...
    let mut rmq = SegmentTree::new(Mode::RangeGet(Op::Max));

    let mut chords = vec![];
    for _ in 0..n {
        let mut a: usize = scanner.cin::<usize>() - 1;
        let mut b: usize = scanner.cin::<usize>() - 1;
        if a > b {
            std::mem::swap(&mut a, &mut b);
        }

        rmq.update_one(a, b as i64);
        debug!("a({}) -> b({})", a, b);
        chords.push((a, b));
    }

    for (a, b) in chords {
        let c = rmq.get_range(a, b);
        debug!("a({}), b({}) -> c({})", a, b, c);
        if c > b as i64 {
            println!("Yes");
            return;
